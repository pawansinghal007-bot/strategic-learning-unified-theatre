...
function readUpdateConfig() {
  const packagedPath = path.join(process.resourcesPath || '', 'config', 'update.json');
  ...
  try {
    ...
    return {
      ...
      healthCheckTimeoutMs: typeof parsed.healthCheckTimeoutMs === 'number' ? parsed.healthCheckTimeoutMs : 30000
    };
  } catch {
...
function writeHealthState(state) {
  fs.writeFileSync(healthStatePath, JSON.stringify(state || {}, null, 2), 'utf8');
}
...
function hasStringField(field) {
  return (payload) => isObjectPayload(payload) && typeof payload[field] === 'string';
}
      ...
      captureResponse: {
        ...
        run: async (payload) => {
          const { ingestBrowserResponseFile } = await import(
            pathToFileURL(path.join(__dirname, '..', 'src', 'browser-bridge.js')).href
          );
