...
function ensureDir(path2) {
  nodeFs.mkdirSync((0, import_node_path.dirname)(path2), { recursive: true });
}
...
function exportAuditLogHtmlReport(filter) {
  ...
  (0, import_node_fs.writeFileSync)(filePath, toHtmlReport(events, verification), "utf8");
  return {
    ok: true,
    format: "html",
    filePath,
...
var import_node_crypto, import_node_fs, import_node_path2, AUDIT_FILE, DEFAULT_AUDIT_STORE;
...
function nextId() {
  return `route_${Date.now()}_${(0, import_node_crypto2.randomBytes)(4).toString("hex")}`;
}
...
var import_node_crypto2, ROUTING_HISTORY_FILE, MAX_HISTORY;
