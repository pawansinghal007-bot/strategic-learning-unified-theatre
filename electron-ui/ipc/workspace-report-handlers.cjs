...
function registerWorkspaceReportHandlers() {
  ipcMain.handle('workspaceReport:save', async (_event, workspaceId, format, filter) => {
    const {
    ...
    if (result.canceled || !result.filePath) {
      return { canceled: true, saved: false, filePath: null, format };
    }

    let content;
    if (format === 'json') {
      content = exportWorkspaceAnalyticsJson(workspaceId, filter ?? undefined);
    } else if (format === 'csv') {
      content = exportWorkspaceAnalyticsCsv(workspaceId, filter ?? undefined);
    } else {
      content = exportWorkspaceAnalyticsHtmlReport(workspaceId, filter ?? undefined);
    }
