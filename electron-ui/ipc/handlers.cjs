...
module.exports = async function register({ ipcMain, dialog, watcher, app }) {
  ...
  ipcMain.handle("llm:ask", async (e, payload) => {
    ...
    let contextBlock = "";
    try {
      const { queryTopK } = await import(
        resolveModule("../../src/llm/qdrant-client.js")
      );
      const { assembleContextFromChunks } = await import(
        resolveModule("../../src/shared/retrieval/context-assembler.js")
      );
      const { countTokens } = await import(
        resolveModule("../../src/shared/retrieval/tokenizer.js")
      );
