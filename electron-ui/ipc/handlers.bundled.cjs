...
module.exports = async function register({ ipcMain, dialog, watcher, app }) {
  const { AccountStore } = await import(resolveModule("../../src/accounts/store.js"));
  const { SecretStore: SecretStoreClass } = await import(resolveModule("../../src/accounts/secret-store.js"));
  const { captureAuthBlob } = await import(resolveModule("../../src/auth-capture.js"));
  const { SwitcherService } = await import(resolveModule("../../src/accounts/switcher.js"));
  const { resolveAuthPath } = await import(resolveModule("../../src/internal/paths.js"));
  const { GitMonitor } = await import(resolveModule("../../src/internal/git-monitor.js"));
  const { Journal } = await import(resolveModule("../../src/internal/journal.js"));
  const { loadConfig, saveConfig } = await import(resolveModule("../../src/internal/config.js"));
  const { probeAccount } = await import(resolveModule("../../src/accounts/health.js"));
  const { getLlmStatus, setupModel, askLocalLlm } = await import(resolveModule("../../src/llm/local-llm.js"));
  ...
  const getAccountAuthInfo = async (account) => {
    try {
      ...
      return {
        ...
        loginUrl: LOGIN_TARGETS[account.agentType] || `https://www.google.com/search?q=${encodeURIComponent(loginQuery)}`,
  ...
  ipcMain.handle("llm:status", async () => {
    ...
    try {
      const { isOpenAiCompatAvailable, listOpenAiCompatModels } = await import(resolveModule("../../src/llm/inference.js"));
...
${userQuery}`
    ...
    try {
      const { isOpenAiCompatAvailable, askOpenAiCompat } = await import(resolveModule("../../src/llm/inference.js"));
      if (await isOpenAiCompatAvailable()) {
