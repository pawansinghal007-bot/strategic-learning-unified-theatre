...
describe('Sprint 54 — drift-history module unit tests', () => {
  it('loadDriftHistory returns empty array for null', async () => {
    const { loadDriftHistory } = await import(
  ...
  it('saveDriftHistory writes file and returns count', async () => {
    const { saveDriftHistory, loadDriftHistory } = await import(
      '../src/security/security-overview/drift-history.js'
    );
  ...
  it('appendDriftHistory creates file and appends one entry', async () => {
    const { appendDriftHistory, loadDriftHistory } = await import(
      '../src/security/security-overview/drift-history.js'
    );
    const tmp = path.join(os.tmpdir(), `drift-history-append-${Date.now()}.json`);
    const entry = {
      id: 'drift-append-1',
      createdAt: 2000,
  ...
  it('appendDriftHistory accumulates entries across multiple calls', async () => {
    ...
    const tmp = path.join(os.tmpdir(), `drift-history-accum-${Date.now()}.json`);
    appendDriftHistory(tmp, { id: 'e1', createdAt: 1, classification: 'clean' });
    appendDriftHistory(tmp, { id: 'e2', createdAt: 2, classification: 'regressed' });
    appendDriftHistory(tmp, { id: 'e3', createdAt: 3, classification: 'mixed' });
    const loaded = loadDriftHistory(tmp);
