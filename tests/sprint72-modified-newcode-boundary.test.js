...
const sprint71StablePath = path.resolve(
  "strategic-learning-unified-theatre-ai-snapshot-sprint71-stable",
);
const sprint71GuardPath = path.resolve(
  "tests/sprint71-newcode-scope-guard.test.js",
);
const sprint70GuardPath = path.resolve(
...
describe("Sprint 72 modified-newcode boundary guard", () => {
  ...
  describe("Coverage and Sonar scripts preserved", () => {
    ...
    it("sonar:scan script exists", () => {
      expect(
        Object.prototype.hasOwnProperty.call(pkg.scripts, "sonar:scan"),
      ).toBe(true);
    });

    it("sonar:qualitygate script exists", () => {
      expect(
        Object.prototype.hasOwnProperty.call(pkg.scripts, "sonar:qualitygate"),
      ).toBe(true);
    });

    it("sonar:preflight script exists", () => {
      expect(
        Object.prototype.hasOwnProperty.call(pkg.scripts, "sonar:preflight"),
      ).toBe(true);
    });
  });
});
