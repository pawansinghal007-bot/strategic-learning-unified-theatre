...
describe("Sprint 35 smoke tests — file existence and surface", () => {
  ...
  it("preload.cjs workspaceRouting methods accept filter", () => {
    ...
    expect(content).toContain("filter-provider");
    expect(content).toContain("filter-start");
    expect(content).toContain("filter-end");
    expect(content).toContain("save-json-report");
    expect(content).toContain("save-html-report");
    expect(content).toContain("workspaceReport.save");
  });
