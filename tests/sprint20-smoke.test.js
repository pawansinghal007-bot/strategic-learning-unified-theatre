...
import { RoutingNoProviderError, ValidationFailedError } from '../src/shared/errors/index.js';
  ...
  it('gateway.ask() throws RoutingNoProviderError when all candidates excluded', async () => {
    await expect(
      gateway.ask({
        ...
        prompt: 'no provider',
        constraints: { excludedProviders: ['groq', 'gemini', 'openai', 'perplexity', 'local'] },
      })
