...
describe('Sprint 54 smoke tests — file surface and IPC/preload/types wiring', () => {
  describe('backend modules — T1', () => {
    it('drift-history.ts exists', () => {
      expect(existsSync(join(root, 'src/security/security-overview/drift-history.ts')))
        .toBe(true);
    });
    ...
    it('auto-scan.ts exists', () => {
      expect(existsSync(join(root, 'src/security/security-overview/auto-scan.ts')))
        .toBe(true);
    });
    ...
    it('auto-scan.ts calls classifyDriftSeverity as a function returning string directly', () => {
      const c = read('src/security/security-overview/auto-scan.ts');
      expect(c).not.toContain('classifyDriftSeverity(driftResult).classification');
      expect(c).not.toContain('classifyDriftSeverity(driftResult)?.classification');
  ...
  describe('compatibility regression — Sprint 44-53 surfaces', () => {
    ...
    it('sprint53 test files still present', () => {
      expect(existsSync(join(root, 'tests/sprint53-cross-surface.test.js')))
        .toBe(true);
      expect(existsSync(join(root, 'tests/sprint53-smoke.test.js')))
        .toBe(true);
    });
    it('sprint52 bulk-triage test still present', () => {
      expect(existsSync(join(root, 'tests/sprint52-bulk-triage.test.js')))
        .toBe(true);
    });
  });
});
