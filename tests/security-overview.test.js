...
describe('Sprint 46 — security overview backend', () => {
  it('loadSecurityBaseline reads findings-wrapper JSON', async () => {
    const { loadSecurityBaseline } = await import(
      '../src/security/security-overview/baseline.js'
    );
  ...
  it('saveSecurityBaseline writes readable file and round-trips', async () => {
    const { saveSecurityBaseline, loadSecurityBaseline } = await import(
      '../src/security/security-overview/baseline.js'
    );
  ...
  it('isSecuritySuppressed matches by fingerprint', async () => {
    const { isSecuritySuppressed } = await import(
      '../src/security/security-overview/suppressions.js'
    );
  ...
  it('loadSecuritySuppressions returns [] for missing file', async () => {
    const { loadSecuritySuppressions } = await import(
      '../src/security/security-overview/suppressions.js'
    );
  ...
  it('buildSecurityOverviewSnapshot aggregates counts correctly', async () => {
    const { buildSecurityOverviewSnapshot } = await import(
      '../src/security/security-overview/schema.js'
    );
    const findings = [
      { kind: 'secret', severity: 'critical', suppressed: false, baselineMatched: false, createdAt: 1000 },
      { kind: 'risk', severity: 'high', suppressed: true, baselineMatched: false, createdAt: 2000 },
      { kind: 'risk', severity: 'medium', suppressed: false, baselineMatched: true, createdAt: 3000 },
    ];
  ...
  it('flattenFindings normalises secrets array', async () => {
    const { flattenFindings } = await import(
      '../src/security/security-overview/normalizer.js'
    );
    const raw = [
      { id: 's1', ruleId: 'AWS', severity: 'CRITICAL', file: 'app.ts', fingerprint: 'fp1' },
    ];
