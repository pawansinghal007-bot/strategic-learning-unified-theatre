const mocks = (globalThis as any).vi.hoisted(() => ({
  ask: vi.fn(),
  runOrchestrator: vi.fn(),
  vectorSearch: vi.fn(),
  searchCode: vi.fn(),
  executeRetrieve: vi.fn(),
  formatVectorResults: vi.fn(),
  formatCodeHits: vi.fn(),
  loggerInfo: vi.fn(),
  loggerError: vi.fn(),
}));

vi.mock("../../src/llm/gateway.ts", async (importOriginal) => {
  const actual = await importOriginal();
  return { ...actual, gateway: { ask: mocks.ask } } as any;
});
...
vi.mock("../../src/shared/retrieval/code-search.js", () => ({
  searchCode: mocks.searchCode,
}));

vi.mock("../../src/shared/retrieval/execute-retrieve.js", () => ({
  executeRetrieve: mocks.executeRetrieve,
}));

vi.mock("../../src/shared/retrieval/format.js", () => ({
  formatVectorResults: mocks.formatVectorResults,
  formatCodeHits: mocks.formatCodeHits,
}));

vi.mock("../../src/shared/logging/logger.ts", async (importOriginal) => {
  const actual = await importOriginal();
  return { ...actual, logger: { info: mocks.loggerInfo, error: mocks.loggerError } } as any;
});
...
describe("mcp tool handlers", () => {
  ...
  it("handleAskLocal returns text on success and logs", async () => {
    const { handleAskLocal } = await import("../../src/mcp/tool-handlers.ts");
    mocks.ask.mockResolvedValueOnce({ outputText: "ok" });

    const res = await handleAskLocal({ prompt: "hi", workspaceId: "w" } as any);
  ...
  it("handleAskLocal returns error when gateway throws", async () => {
    const { handleAskLocal } = await import("../../src/mcp/tool-handlers.ts");
    mocks.ask.mockRejectedValueOnce(new Error("fail"));

    const res = await handleAskLocal({ prompt: "hi" } as any);

    expect(res.isError).toBeTruthy();
    expect(res.content[0].text).toContain("Error: fail");
    expect(mocks.loggerError).toHaveBeenCalled();
  });

  it("handleCodeReview returns finalOutput on success and handles orchestrator error", async () => {
    const { handleCodeReview } = await import("../../src/mcp/tool-handlers.ts");
    mocks.runOrchestrator.mockResolvedValueOnce({ finalOutput: "rev" });

    const ok = await handleCodeReview({ filePath: "f" } as any);
    expect(ok.isError).toBeFalsy();
    expect(ok.content[0].text).toBe("rev");

    mocks.runOrchestrator.mockResolvedValueOnce({ error: "bad" });
    const err = await handleCodeReview({ filePath: "f" } as any);
    expect(err.isError).toBeTruthy();
    expect(err.content[0].text).toContain("Review failed");
  });

  it("handleListTools returns description", async () => {
    const { handleListTools } = await import("../../src/mcp/tool-handlers.ts");
    const res = await handleListTools();
    expect(res.content[0].text).toContain("Available MCP tools");
  });

  it("handleVectorSearch handles empty and formatted results", async () => {
    const { handleVectorSearch } = await import("../../src/mcp/tool-handlers.ts");
    mocks.vectorSearch.mockResolvedValueOnce([]);
    const empty = await handleVectorSearch({ query: "q" } as any);
    expect(empty.content[0].text).toBe("No results found.");

    mocks.vectorSearch.mockResolvedValueOnce([{ id: 1 } as any]);
    mocks.formatVectorResults.mockReturnValueOnce("fmt");
    const ok = await handleVectorSearch({ query: "q" } as any);
    expect(ok.content[0].text).toBe("fmt");
  });

  it("handleSearchCode handles empty and formatted hits", async () => {
    const { handleSearchCode } = await import("../../src/mcp/tool-handlers.ts");
    mocks.searchCode.mockResolvedValueOnce([]);
    const empty = await handleSearchCode({ pattern: "p" } as any);
    expect(empty.content[0].text).toBe("No matches found.");

    mocks.searchCode.mockResolvedValueOnce([{ file: "x" } as any]);
    mocks.formatCodeHits.mockReturnValueOnce("codefmt");
    const ok = await handleSearchCode({ pattern: "p" } as any);
    expect(ok.content[0].text).toBe("codefmt");
  });

  it("handleRetrieve returns error when executeRetrieve reports error and handles success", async () => {
    const { handleRetrieve } = await import("../../src/mcp/tool-handlers.ts");
    mocks.executeRetrieve.mockResolvedValueOnce({ error: "nope" });
    const err = await handleRetrieve({ query: "q" } as any);
    expect(err.isError).toBeTruthy();
    expect(err.content[0].text).toBe("nope");

    mocks.executeRetrieve.mockResolvedValueOnce({ text: "got" });
    const ok = await handleRetrieve({ query: "q" } as any);
    expect(ok.content[0].text).toBe("got");
  });
});
/**
