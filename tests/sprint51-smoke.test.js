...
describe('Sprint 51 smoke tests — timeline and snapshot files', () => {
  ...
  it('CURRENT_ACTIVE_SNAPSHOT.md points to sprint51-stable', () => {
    const text = read('CURRENT_ACTIVE_SNAPSHOT.md').trim();
    expect(text).toBe('strategic-learning-unified-theatre-ai-snapshot-sprint51-stable');
  });

  it('sprint51-stable snapshot file exists', () => {
    expect(
      existsSync(
        join(root, 'strategic-learning-unified-theatre-ai-snapshot-sprint51-stable'),
      ),
    ).toBe(true);
  });
});
