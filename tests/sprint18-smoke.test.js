import { providerRequestSchema, providerResponseSchema, providerHealthSchema, tokenChunkSchema } from '../src/shared/schemas/provider.schema.js';
import { DomainError, ProviderQuotaError, RoutingNoProviderError, MemoryNotFoundError, ValidationFailedError } from '../src/shared/errors/index.js';
import { describe, it, expect } from 'vitest';

describe('Sprint 18 smoke tests', () => {
  it('providerRequestSchema validates a minimal request', () => {
    const result = providerRequestSchema.safeParse({ requestId: 'r1', prompt: 'hello' });
    expect(result.success).toBe(true);
  });

  it('providerRequestSchema rejects empty prompt', () => {
    const result = providerRequestSchema.safeParse({ requestId: 'r1', prompt: '' });
