...
const sprint70StablePath = path.resolve(
  "strategic-learning-unified-theatre-ai-snapshot-sprint70-stable",
);
...
describe("Sprint 71 new-code scope guard", () => {
  ...
  describe("Sprint 70 root-cause preserved", () => {
    ...
    it("sprint70 stable confirms source files were not modified in Sprint 60+", () => {
      expect(sprint70Stable).toContain(
        "were not modified in any Sprint 60+ commit",
      );
    });

    it("master-instructions records sonar-project.properties was not changed", () => {
      expect(instructions).toContain(
        "sonar-project.properties confirmed already correctly configured",
      );
    });
  });
