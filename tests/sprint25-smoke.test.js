...
describe("Sprint 25 — Provider Telemetry IPC and Dashboard", () => {
  ...
  describe("Preload bridge — providerTelemetry exposure", () => {
    ...
    it("exposes getStatus method", () => {
      ...
      expect(preloadContent).toContain(
        'ipcRenderer.invoke("providerTelemetry:getStatus"',
      );
    });
    ...
    it("exposes getUsage method", () => {
      ...
      expect(preloadContent).toContain(
        'ipcRenderer.invoke("providerTelemetry:getUsage"',
      );
    });
    ...
    it("exposes resetHealth method", () => {
      ...
      expect(preloadContent).toContain(
        'ipcRenderer.invoke("providerTelemetry:resetHealth"',
      );
    });
    ...
    it("exposes resetUsage method", () => {
      ...
      expect(preloadContent).toContain(
        'ipcRenderer.invoke("providerTelemetry:resetUsage"',
      );
    });
    ...
    it("exposes resetAll method", () => {
      ...
      expect(preloadContent).toContain(
        'ipcRenderer.invoke("providerTelemetry:resetAll"',
      );
    });
  });
  ...
  describe("IPC handler registration", () => {
    ...
    it("registers getStatus handler", () => {
      expect(handlerContent).toContain(
        'ipcMain.handle("providerTelemetry:getStatus"',
      );
    });

    it("registers getUsage handler", () => {
      expect(handlerContent).toContain(
        'ipcMain.handle("providerTelemetry:getUsage"',
      );
    });

    it("registers resetHealth handler", () => {
      expect(handlerContent).toContain(
        'ipcMain.handle("providerTelemetry:resetHealth"',
      );
    });

    it("registers resetUsage handler", () => {
      expect(handlerContent).toContain(
        'ipcMain.handle("providerTelemetry:resetUsage"',
      );
    });

    it("registers resetAll handler", () => {
      expect(handlerContent).toContain(
        'ipcMain.handle("providerTelemetry:resetAll"',
      );
    });
    ...
    it("requires provider-usage module", () => {
      expect(handlerContent).toContain(
        'require("../../src/llm/provider-usage.js")',
      );
    });
  });
