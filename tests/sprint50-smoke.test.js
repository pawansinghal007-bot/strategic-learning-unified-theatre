...
describe('Sprint 50 smoke tests — consolidated T1-T3 surface', () => {
  describe('backend surface — T1', () => {
    it('TRIAGE_STATUSES includes all four values', async () => {
      const { TRIAGE_STATUSES } = await import('../src/security/security-overview/schema.js');
    ...
    it('normalizeTriageStatus returns open for null', async () => {
      const { normalizeTriageStatus } = await import('../src/security/security-overview/normalizer.js');
    ...
    it('isTriageStatusFinal returns true for fixed', async () => {
      const { isTriageStatusFinal } = await import('../src/security/security-overview/triage.js');
    ...
    it('classifyDriftSeverity returns clean for empty lists', async () => {
      const { classifyDriftSeverity } = await import('../src/security/security-overview/drift.js');
      expect(classifyDriftSeverity({ introduced: [], resolved: [], persistent: [] })).toBe('clean');
    });
    it('classifyDriftSeverity returns regressed when only introduced', async () => {
      const { classifyDriftSeverity } = await import('../src/security/security-overview/drift.js');
      expect(classifyDriftSeverity({ introduced: [{}], resolved: [], persistent: [] })).toBe('regressed');
    });
    it('classifyDriftSeverity returns improved when only resolved', async () => {
      const { classifyDriftSeverity } = await import('../src/security/security-overview/drift.js');
      expect(classifyDriftSeverity({ introduced: [], resolved: [{}], persistent: [] })).toBe('improved');
    });
    it('classifyDriftSeverity returns mixed when both non-empty', async () => {
      const { classifyDriftSeverity } = await import('../src/security/security-overview/drift.js');
      expect(classifyDriftSeverity({ introduced: [{}], resolved: [{}], persistent: [] })).toBe('mixed');
    });
  });
