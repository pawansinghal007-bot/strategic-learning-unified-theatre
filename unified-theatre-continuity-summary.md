...
> **Last updated 2026-08-06 (RAG production-readiness remediation).** The current RAG stack is centered on a single path: ingestion in `src/knowledge/ingest/ingest-repository.js`, embeddings in `src/knowledge/ingest/embedder.js` (retry-aware batching, persistent cache, structured logging), storage and retrieval in `src/llm/qdrant-client.js`, hybrid fusion in `src/llm/hybrid-search.js`, optional reranking in `src/llm/reranker.js`, and budget-aware context assembly in `src/shared/retrieval/context-assembler.js`. `src/shared/retrieval/vector-client.ts` is intentionally a thin delegate rather than a second implementation. This branch fixes the earlier architecture drift around embedding/retrieval consolidation, but it still does not represent a full production-ready deployment claim.
