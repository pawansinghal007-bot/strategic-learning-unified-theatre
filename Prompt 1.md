refer Architecture summary across sprints in folder E:\VS Code Agent\Solution before development 
 ...
 You are building "vscode-rotator", a Node.js CLI tool for hands-free VS Code account rotation. 
 
 SPRINT 1 SCOPE — account store + CLI skeleton. 
 
 Constraints: 
 • Node.js >=18, ESM modules, no build step required 
 • Store path: ~/.vscode-rotator/accounts.enc (AES-256-GCM, key derived from machine-id via crypto.scryptSync) 
 ...
 5. package.json     — scripts: start, test; bin entry "vscode-rotator" 
