...
export class StorageMonitor {
  ...
  async maxAgeDays() {
    const config = await this.getConfig();
    return typeof config.storageIndexMaxAgeDays === "number"
      ? config.storageIndexMaxAgeDays
      : 30;
  }
  ...
  async appendChanges(changes) {
    ...
    for (const change of tracked) {
      ...
      const size =
        change.event === "unlink"
          ? 0
          : (change.size ?? (await fileSize(absolutePath)));
      const entry = {
        ts,
        path: absolutePath,
        event: change.event,
    ...
    const ingestibleChanges = tracked.filter((change) =>
      isIngestible(change.path),
    );
  ...
  async watch() {
    ...
    const labels = new Map(
      normalizedEntries.map((entry) => [entry.path, entry.label]),
    );
    const ignored = (filePath) => {
      ...
      const match = normalizedEntries
        .filter(
          (entry) =>
            absolute === entry.path ||
            absolute.startsWith(entry.path + path.sep),
        )
