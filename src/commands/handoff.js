  ...
  const result = await setTokenBudget(sprintId, {
    tokensUsed:
      options.tokensUsed === undefined
        ? sprint.tokensUsed
        : parsePositiveInt(options.tokensUsed, "--tokens-used"),
    tokensLimit:
      options.tokensLimit === undefined
        ? sprint.tokensLimit
        : parsePositiveInt(options.tokensLimit, "--tokens-limit"),
  });
...
export function bindHandoffCommands(program) {
  const handoff = program
    .command("handoff")
    .description("Track agent sprint handoff state");
  ...
  handoff
    ...
    .option(
      "--agent <agent>",
      "Agent name (claude|chatgpt|gemini|perplexity|other)",
      "other",
    )
  ...
  handoff
    ...
    .action(async (sprintId, options) => {
      ...
      try {
        ...
        ({ sprint, warnings } = await handleUpdateTokenBudget(
          sprintId,
          options,
          sprint,
          warnings,
  ...
  handoff
    ...
    .requiredOption(
      "--status <status>",
      "Sprint status (complete|paused|exhausted)",
    )
    .action(async (sprintId, options) => {
      ...
      try {
        const sprint = await closeSprint(
          sprintId,
          parseHandoffStatus(options.status),
        );
  ...
  handoff
    ...
    .action(async (sprintId) => {
      try {
        ...
        const resumePrompt =
          sprint.resumePrompt || generateResumePrompt(sprint);
