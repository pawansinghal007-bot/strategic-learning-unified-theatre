...
export function initDashboard() {
  ...
  document
    ...
    .addEventListener("click", async () => {
      const rows =
        await globalThis.workspaceRouting.globalAnalytics(getFilter());
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      reportOut.textContent =
        await globalThis.workspaceRouting.exportHtmlReport(id, getFilter());
    });

  document.getElementById("save-json").addEventListener("click", async () => {
    const id = wsId();
    if (!id) return;
    const result = await globalThis.workspaceReport.save(
      id,
      "json",
      getFilter(),
    );
  ...
  (function () {
    ...
    const secVal = document.querySelector(
      '[data-testid="proof-security-value"]',
    );
    ...
    setDemoPersistenceState(
      "Standby",
      "Demo persistence state is standing by.",
    );
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      const result = await globalThis.workspaceQuota.set(
        id,
        payload,
        undefined,
      );
  ...
  async function compareSecurityBaseline() {
    ...
    try {
      ...
      if (currentEl)
        currentEl.textContent = String(result?.counts?.current ?? 0);
      if (baselineEl)
      ...
      if (loadedEl)
        loadedEl.textContent = result?.baselineLoaded ? "yes" : "no";
