...
export class WatcherDaemon extends EventEmitter {
  ...
  async start(
    ...
    try {
      ...
      this.interval =
        typeof cfg?.pollIntervalMs ===
        "number"
          ? cfg.pollIntervalMs
          : defaultPollIntervalMs;

      this.cooldownMs =
        typeof cfg?.cooldownMs ===
        "number"
          ? cfg.cooldownMs
          : 15 * 60 * 1000;

      const gitInterval =
        typeof cfg?.gitPollIntervalMs ===
        "number"
          ? cfg.gitPollIntervalMs
          : this.interval;

      await this._setupGitMonitoring(
        cfg,
        gitInterval
      );

      await this._setupEnhanceLoop(
        cfg
      );

      await this._setupCaptureLoop(
        cfg
      );
  ...
  async _safeTick(cfg) {
    ...
    try {
      ...
      log.error(
        "watcher.tick.failure",
        {
          error: err,
          code:
            err?.code ??
            "ROTATOR_WATCHER_TICK_FAILED"
        }
      );
  ...
  async _tick(cfg) {
    const cleared =
      await this.scheduler.clearExpired();
    ...
    const until =
      await this.scheduler.setCooldown(
        current.id,
        durationMs
      );
  ...
  async _setupGitMonitoring(
    ...
    this.gitMonitor.watchAll(
      watchedRepos,
      gitInterval
    );

    this.gitMonitor.removeAllListeners(
      "warn"
    );
  ...
  async _setupEnhanceLoop(cfg) {
    ...
    const platform =
      enhanceConfig.platform ??
      "chatgpt";

    let running = false;

    const runEnhanceCycle =
      async () => {
        if (
          !this.running ||
          running
              ...
              await this.journal.append(
                {
                  type: "CAPTURE",
                  detail:
                    `${platform} | ${result.filename ?? result.filePath ?? "no-file"}`
                }
              );
