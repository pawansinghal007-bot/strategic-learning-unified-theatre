...
export class ProviderQuotaError extends DomainError {
  constructor(message = 'Provider quota exceeded', details?: Record<string, unknown>) {
    super('PROVIDER_QUOTA_EXCEEDED', message, { retryable: true, details });
  }
}

export class ProviderAuthError extends DomainError {
  constructor(message = 'Provider authentication failed', details?: Record<string, unknown>) {
...
export class ProviderBadResponseError extends DomainError {
  constructor(message = 'Provider returned an invalid response', details?: Record<string, unknown>) {
