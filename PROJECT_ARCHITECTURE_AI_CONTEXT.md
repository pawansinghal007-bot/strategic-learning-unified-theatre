...
- MCP stdio protocol handshake verified via scripts/verify-mcp-stdio.mjs with
  ...
  Confidence upgraded from [INFERRED] to [CONFIRMED] per Sprint 101 verification.
...
- MCP migration: [CONFIRMED] the migration from the older server concept to the
  current McpServer + Zod-schema pattern is now reflected in source and verified
  via live stdio handshake in scripts/verify-mcp-stdio.mjs (Sprint 101).
