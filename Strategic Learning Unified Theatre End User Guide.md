...
Strategic Learning Unified Theatre is a local desktop tool for managing and switching between Visual Studio Code account profiles, monitoring Git repositories, and viewing progress logs. It is designed for business users who need to maintain several VS Code authorization profiles and follow workspace health without deep technical setup.
...
- The app stores settings in local files such as `~/.vscode-rotator/config.json` (runtime storage location remains unchanged for backward compatibility).
...
- Use strong local machine security for files stored under `~/.vscode-rotator` (all local storage for Strategic Learning Unified Theatre).
