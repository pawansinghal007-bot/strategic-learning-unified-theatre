...
export default function RobotFramework() {
  ...
  useEffect(() => {
    ...
    setOutput("");
    setStatus(
      "Select a suite or file, then run tests or generate a Robot skeleton from a source file.",
    );
    loadRobotFiles();
  }, []);
  ...
  const runSelectedRobotFile = async () => {
    ...
    try {
      ...
      setSummary(result);
      setStatus(
        `Completed ${selectedRobotFile} with exit code ${result.exitCode}`,
      );
      setOutput(JSON.stringify(result, null, 2));
    } catch (err) {
  ...
  const runTddCheck = async () => {
    ...
    try {
      const result = await globalThis.rotator.robot.tddCheck({
        graceMs: 60000,
      });

      setSummary({
        passed: result.length === 0 ? 1 : 0,
  ...
  const generateSkeleton = async () => {
    ...
    try {
      const generatedPath =
        await globalThis.rotator.robot.generateSkeleton(selectedSourceFile);
    ...
    <div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Run Robot suites, choose a test file, or generate a Robot skeleton
            from a source file.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-4 mb-4">
        <div className="xl:col-span-2 bg-white dark:bg-gray-800 rounded shadow p-4">
          <div className="mb-4">
            <label htmlFor="robot-suite" className="block text-sm font-medium">
              Suite
            </label>
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={runSelectedRobotFile}
                disabled={running || !selectedRobotFile}
        ...
        <div className="bg-white dark:bg-gray-800 rounded shadow p-4">
          ...
          <button
            ...
            disabled={running || !selectedSourceFile}
          ...
          {generatedRobotFile && (
            <div className="mt-4 text-sm text-green-600 dark:text-green-400 break-words">
              ...
              <div className="mt-2">
                <button
                  onClick={() => openBrowserFile(generatedRobotFile)}
                  disabled={running}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3 mb-4">
        <div className="lg:col-span-1 bg-white dark:bg-gray-800 rounded shadow p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={loadRobotFiles}
              disabled={running}
              className="text-sm px-3 py-1 bg-slate-600 text-white rounded"
            >
              Refresh
            </button>
          </div>
          ...
          <div className="h-64 overflow-auto border border-gray-200 dark:border-gray-700 rounded p-2 bg-gray-50 dark:bg-gray-900">
            {robotFiles.length === 0 ? (
              ...
              robotFiles.map((file) => (
                <div
                  key={file}
                  className="mb-2 p-2 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-900"
                >
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded shadow p-4">
          ...
          {browserSelectedFile ? (
            <>
              <div className="text-sm text-gray-700 dark:text-gray-300 mb-3 break-words">
                {browserSelectedFile}
              </div>

              <div className="h-80 overflow-auto rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-3 text-xs whitespace-pre-wrap break-words">
                {previewContent || "No preview available."}
              </div>
            </>
          ) : (
            <div className="text-sm text-gray-500">
              Select a Robot file to preview its contents.
            </div>
          )}
        </div>
      </div>
